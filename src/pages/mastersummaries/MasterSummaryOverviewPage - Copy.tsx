...
import { ArrowLeft, MapPin, School, Users, Building2, Layers, Landmark, MapPinned, GraduationCap } from "lucide-react";
...
const MasterSummaryOverviewPage = () => {
  ...
  const fetchMasterSummaryData = async () => {
    ...
    try {
      ...
      if (!msData) {
        showError("Master summary not found");
        return;
      }
      ...
      setSummaryDetails(detailsData || []);
    } catch (e: any) {
      showError(e.message || "Failed to load overview data");
      console.error(e);
    } finally {
      setLoading(false);
    }
  ...
  const totals = useMemo(() => {
    ...
    summaryDetails.forEach(detail => {
      const regionName = detail.region || "Unknown";
      ...
      if (!breakdown[regionName].districts[districtName]) {
        breakdown[regionName].districts[districtName] = { 
          totalRegistered: 0, 
          totalCenters: 0, 
          totalStreams: 0, 
          sCount: 0, 
      ...
      const district = breakdown[regionName].districts[districtName];
      district.totalRegistered += registered;
      district.totalCenters += 1;
      ...
      if (isSecondary) {
        if (isPrivate) {
          ...
          district.sCount++;
          district.sRegistered += registered;
        }
      }

      breakdown[regionName].totalRegistered += registered;
      breakdown[regionName].totalStreams += streams;
      breakdown[regionName].totalCenters += 1;
    });

    return Object.values(breakdown).sort((a: any, b: any) => a.name.localeCompare(b.name));
          ...
          <Card className="border-none shadow-sm bg-white rounded-[2rem] p-8 flex flex-col justify-between hover:shadow-xl transition-all border border-slate-100 group">
            ...
            <div>
              ...
              <h3 className="text-3xl font-[1000] text-slate-900 tracking-tighter leading-none">
                <CountUp value={stats.registered}/>
              </h3>
            </div>
          </Card>
          <Card className="border-none shadow-sm bg-white rounded-[2rem] p-8 flex flex-col justify-between hover:shadow-xl transition-all border border-slate-100 group">
          ...
          {totals
            ...
            .map((region: any) => (
              <AccordionItem 
                key={region.name} 
                value={region.name} 
                className="border-none bg-white rounded-[2rem] shadow-sm hover:shadow-md transition-all overflow-hidden ring-1 ring-slate-200/50"
              >
                <AccordionTrigger className="px-8 py-7 hover:no-underline [&[data-state=open]]:bg-slate-50 transition-colors">
                  <div className="flex flex-col md:flex-row md:items-center justify-between w-full pr-6 gap-6">
                    <div className="flex items-center gap-5">
                      <div className="h-14 w-14 rounded-3xl bg-slate-900 flex items-center justify-center text-xl font-black text-white shadow-lg">
                        {region.name[0]}
                      </div>
                      <div className="text-left">
                        <span className="font-black text-2xl text-slate-900">{region.name}</span>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                          {Object.keys(region.districts).length} Districts
                        </p>
                      </div>
                    </div>
                ...
                <AccordionContent className="px-8 pb-10 pt-4 bg-slate-50/50">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                    {Object.entries(region.districts).map(([dName, dStats]: [string, any]) => (
                      <div 
                        key={dName} 
                        className="p-7 rounded-[2.5rem] bg-white border border-slate-200/60 shadow-sm hover:shadow-xl hover:translate-y-[-2px] transition-all duration-300"
                      >
                          ...
                          <div className="flex items-center justify-between px-1 pt-2 border-t border-slate-50">
                            <div>
                              ...
                              <p className="text-2xl font-black text-emerald-600 tracking-tighter">
                                <CountUp value={dStats.totalRegistered}/>
                              </p>
                            </div>
                            <div className="text-right">
                              ...
                              <p className="text-2xl font-black text-purple-600 tracking-tighter">
                                <CountUp value={dStats.totalStreams}/>
                              </p>
                            </div>
                          </div>
                        </div>
...
const RegionIndicator = ({ label, value, icon, color }: any) => {
  ...
  return (
    <div className="hidden sm:flex flex-col items-end">
      ...
      <p className={`font-black text-xl tracking-tight ${colors[color]}`}>
        <CountUp value={value} />
      </p>
    </div>
  );
