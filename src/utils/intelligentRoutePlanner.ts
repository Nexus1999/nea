...
export function generateIntelligentRoutes(
  ...
  // =========================================================================
  {
    ...
    if (allSH.length > 0) {
      ...
      if (total <= MAX_BOXES_PER_TT) {
        stage("Southern Highlands (Full)", ["IRINGA", "NJOMBE", "RUVUMA", "MBEYA", "SONGWE", "RUKWA"], allSH, 3, TT_ONLY);
      } else {
        const morogoro = demand("MOROGORO");
        ...
        const eastP2fits = !!morogoro && eastP2.length >= 2 && sum(eastP2) <= MAX_BOXES_PER_TT;

        if (westP2fits && eastP2fits) {
          stage("Southern Highlands (West)", ["IRINGA", "MBEYA", "SONGWE", "RUKWA"], westP2, 3, TT_ONLY);
          stage("Southern Highlands (East + Morogoro)", eastP2.map(d => d.region), eastP2, 3, TT_ONLY);
        } else {
  ...
  // =========================================================================
  {
    ...
    if (present.length > 0) {
      if (sum(present) <= MAX_BOXES_PER_TT) {
        ...
        stage("Northern (Core TT + Tanga Truck)", ["TANGA", "KILIMANJARO", "ARUSHA", "MANYARA"], [...coreList, tanga], 4, ttPlusExtraTruck("TANGA"));
      } else if (akSum <= MAX_BOXES_PER_TT) {
        if (tanga) {
          stage("Northern (Arusha/Kili TT + Tanga Truck)", ["TANGA", "KILIMANJARO", "ARUSHA"], [...akList, tanga], 4, ttPlusExtraTruck("TANGA"));
        } else {
  ...
  while (pool.size > 0) {
    ...
    stage(batch.length === 1 ? `${firstReg} Route` : "Mixed Route", batch.map(d => d.region), batch, 99, TT_ONLY);
  }
