...
const EditSupervisorDrawer = ({ isOpen, onClose, onRefresh, supervisorId }: EditSupervisorDrawerProps) => {
  ...
  const loadSupervisorData = async () => {
    ...
    try {
      const { data: regionData } = await supabase.from('regions').select('*').order('region_name');
      ...
      if (sup) {
        setFormData({
          ...
          phone: sup.phone || '',
          tsc_no: sup.tsc_no || '',
          cheque_no: sup.cheque_no || '',
          region: sup.region || '',
          district: sup.district || '',
          center_no: sup.center_no || '',
        ...
        <div className="px-6 py-4 border-b bg-slate-50/50">
          <SheetHeader>
            <div className="flex items-center gap-3">
              <div className="p-1.5 bg-black text-white rounded-md"><Edit3 className="h-4 w-4" /></div>
              <SheetTitle className="text-lg font-bold">Edit Supervisor Details</SheetTitle>
            </div>
          </SheetHeader>
        </div>
