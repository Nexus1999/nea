...
// Define the schema for the multiplier form
...
const SubjectMultiplierModal: React.FC<SubjectMultiplierModalProps> = ({ open, onOpenChange, subject, onSuccess }) => {
  ...
  useEffect(() => {
    if (subject) {
      ...
      // Reset form when closing or if no subject is provided
      form.reset({
        id: 0,
        normal_booklet_multiplier: 0.0,
        graph_booklet_multiplier: 0.0,
      ...
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Calculator className="h-6 w-6 text-neas-green" /> Edit Multipliers
          </DialogTitle>
          <DialogDescription>
