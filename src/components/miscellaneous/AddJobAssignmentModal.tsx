...
export const AddJobAssignmentModal = ({ open, onOpenChange, onSuccess }: any) => {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="px-6 pb-6 space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-1">
                  ...
                  <FormControl><Input placeholder="Enter name..." className="h-9" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <div className="flex justify-end pt-2 gap-3">
              ...
              <Button type="submit" className="h-9 px-6 bg-black hover:bg-gray-800 text-white" disabled={loading}>
                {loading ? <Loader2 className="animate-spin h-4 w-4" /> : <><Save className="mr-2 h-4 w-4" /> Create Assignment</>}
              </Button>
            </div>
          </form>
        </Form>
