...
const UserForm: React.FC<UserFormProps> = ({ isOpen, onClose, user, onSuccess }) => {
  ...
  return (
    <>
      ...
      <ConfirmDialog
        isOpen={confirmOpen}
        title={isEditing ? "Update User Details?" : "Register New User?"}
        message={isEditing ? `Are you sure you want to save changes for ${formData.username}?` : `Are you sure you want to register ${formData.username} as a new user?`}
        confirmText={isEditing ? "Yes, Update" : "Yes, Register"}
