        ...
        <SheetContent className="sm:max-w-[650px] p-0 flex flex-col bg-white border-none shadow-2xl">
          ...
          >
            {!initialData && (
              <Button
                ...
                className="w-full h-12 bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase text-[10px] tracking-widest rounded-xl flex items-center justify-center gap-2"
              >
                <Sparkles className="h-4 w-4" />
                AI Smart Route Suggester — Generate from Box Counts
              </Button>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <Label className="text-slate-700 font-semibold flex items-center gap-2">
                  <Navigation className="h-4 w-4 text-slate-400" /> Msafara Name / Destination
                </Label>
                <Select
                  onValueChange={handleMsafaraChange}
                  value={watch("name")}
                >
              ...
              <div className="space-y-2">
                ...
                />
                {errors.loadingDate && (
                  <p className="text-xs text-red-500">{errors.loadingDate.message}</p>
                )}
              </div>

              <div className="space-y-2">
                <Label className="text-slate-700 font-semibold flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-slate-400" /> Start Date (Departure)
                </Label>
                <Input
                  type="date"
                  {...register("startDate")}
            ...
            <div className="space-y-4 bg-slate-50 p-6 rounded-2xl border border-slate-100">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    appendVehicle({ type: "LORRY_HORSE_AND_TRAILER", quantity: 1 })
                  }
                  className="h-7 text-blue-600 font-bold uppercase text-[9px] tracking-widest"
                >
              ...
              <div className="space-y-3">
                {vehicleFields.map((field, index) => (
                  <div
                    key={field.id}
                    className="flex items-end gap-3 bg-white p-3 rounded-xl border border-slate-100 shadow-sm"
                  >
                    <div className="flex-1 space-y-1.5">
                      ...
                      <Select
                        onValueChange={(val) =>
                          setValue(`vehicles.${index}.type`, val)
                        }
                    ...
                    <div className="w-24 space-y-1.5">
                      ...
                      <Input
                        type="number"
                        {...register(`vehicles.${index}.quantity`)}
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-900 flex items-center gap-2">
                <MapPin className="w-4 h-4 text-blue-600" /> Path Stops (In Route Order)
              </h3>
              ...
              <div className="space-y-4">
                {regionFields.map((field, index) => (
                  <div
                    key={field.id}
                    className="p-5 rounded-2xl border border-slate-100 bg-white shadow-sm space-y-4 relative group"
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <Select
                          onValueChange={(val) => handleRegionChange(index, val)}
                          value={watch(`regions.${index}.name`)}
                        >
                          <SelectTrigger className="h-9 rounded-lg border-slate-200">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          type="date"
                          {...register(`regions.${index}.deliveryDate`)}
                          className="h-9 rounded-lg border-slate-200"
                        />
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          {...register(`regions.${index}.receivingPlace`)}
                          className="h-9 rounded-lg border-slate-200"
                    ...
                    {regionFields.length > 1 && (
                      <Button
                        ...
                        className="absolute -top-2 -right-2 h-7 w-7 rounded-full bg-white border border-slate-100 text-slate-300 hover:text-red-600 shadow-sm"
                      >
              ...
              <Button
                ...
                className="w-full h-11 rounded-xl border-dashed border-slate-200 text-slate-500 font-bold uppercase text-[10px] tracking-widest hover:bg-slate-50 hover:text-blue-600 hover:border-blue-200 transition-all"
              >
          ...
          <div className="px-6 py-4 border-t bg-slate-50 flex justify-end gap-3 shrink-0">
            ...
            <Button
              ...
              disabled={loading}
              className="bg-slate-900 hover:bg-slate-800 text-white font-black uppercase text-[10px] tracking-widest px-8 rounded-xl h-11 min-w-[140px]"
            >
              {loading ? (
