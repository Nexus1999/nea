...
import { Building, Plus, Search, Filter, MapPin, ExternalLink, ShieldCheck } from 'lucide-react';
...
const SecondarySchools = () => {
  ...
  const columns: Column<any>[] = [
    ...
    { 
      header: 'Category', 
      cell: (s: any) => (
        <span className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${s.category === 'GOVERNMENT' ? 'bg-indigo-50 text-indigo-700' : 'bg-purple-50 text-purple-700'}`}>
          {s.category || 'N/A'}
        </span>
      ) 
    },
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 bg-blue-600 rounded-[1.25rem] flex items-center justify-center text-white shadow-lg shadow-blue-200">
            <Building className="h-8 w-8" />
          </div>
          <div>
            <h1 className="text-2xl font-black text-gray-900">Secondary Schools</h1>
            <p className="text-sm text-gray-500 font-medium">Registry for CSEE & ACSEE examination centers.</p>
          </div>
        </div>
