...
import {
  Shield, Users, Lock, FileText, Activity, AlertTriangle,
  ShieldCheck, ShieldAlert, TrendingUp, Clock, Terminal, Fingerprint
} from 'lucide-react';
...
const SecurityOverview = () => {
  const { data: usersData } = useQuery({
    queryKey: ['security-users', 1],
    queryFn: () => securityService.getUsers({ limit: 100 }).then(r => r.data),
  });
  ...
  const activeSessions = sessionsData?.data?.filter((s: any) => s.status === 'ACTIVE').length ?? 0;
  const totalRoles = rolesData?.data?.length ?? 0;
  const totalPerms = permsData?.data?.length ?? 0;
  ...
  return (
    <div className="space-y-10 pb-12">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard title="Active Access" value={activeSessions} icon={Fingerprint} color="text-emerald-600" subtitle="Live Sessions" delay={0.1} href="/dashboard/security/sessions" />
        ...
        >
          ...
          <div className="space-y-4">
            {[
              ...
              { label: 'Access History', href: '/dashboard/security/sessions', icon: Clock, desc: 'Live session monitor' },
            ].map((item) => (
